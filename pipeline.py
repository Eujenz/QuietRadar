...
class SimpleLLMDistiller:
    ...
    def distill(self, candidates: List[Dict[str, Any]], profile: Dict[str, Any], top_k: int = 7) -> List[Dict[str, Any]]:
        if not candidates:
            return []
        ...
        system_prompt = f"""你是一位具備資深技術背景的「繁體中文科技電子報主編」。
你的任務是：依據讀者的關注領域，從候選文章中篩選出最精華的 {top_k} 則，撰寫成一份「講人話、無 AI 罐頭套話、專業具體」的科技情報電子報。
...
1. 先保事實，再去 AI 味，講大白話：
   ...
   - 推薦理由與重點必須深入具體：說明技術突破點、解決了什麼痛點、架構或效能有何變化，不重複贅述標題。
2. 台灣在地化用語校正：
...
[
  {{
    ...
    "source_name": "來源名稱",
    "reason": "首度達到重大網路安全能力門檻，能自主尋找 Zero-day 漏洞並模擬完整攻防路徑。",
    "bullets": [
      "支援全自動化程式碼弱點探測，加速資安團隊修補時效",
      "具備端到端滲透測試驗證能力，防禦端可提前佈署特徵碼"
    ]
  }}
]
"""
...
# ==========================================
def generate_newsletter_file(items: List[Dict[str, Any]], filepath: str = "latest_newsletter.md"):
    ...
    lines = [
        f"# ⚡ QuietRadar 降噪科技電子報",
        f"> 出刊時間：{now_str} | 本期精選：{len(items)} 則 | 去 AI 雜訊率：約 80%\n",
        "---",
    ]

    global_idx = 1
    for source_name, source_items in grouped_by_source.items():
        lines.append(f"\n## 📰 【{source_name}】({len(source_items)} 則)")
        for item in source_items:
            ...
            lines.append(f"### {global_idx}. [{title}]({url})")
            if reason:
                lines.append(f"**💡 為什麼值得讀**：{reason}\n")
            lines.append("**📌 核心乾貨重點**：")
            for b in bullets:
...
class SimpleBarkNotifier:
    ...
    def send_digest(self, items: List[Dict[str, Any]]) -> bool:
        ...
        # 構建 Bark 原生 Markdown 排版（純標題超連結清單，無摘要）
        now_str = datetime.now().strftime("%m/%d %H:%M")
        md_lines = [f"🎯 **QuietRadar 降噪情報** ({now_str})\n"]

        global_idx = 1
        for source_name, source_items in grouped_by_source.items():
            md_lines.append(f"### 📌 {source_name} ({len(source_items)})")
            for item in source_items:
                title = item.get("title", "").strip()
                url = item.get("original_url", "").strip()

                # 原生 Markdown 格式：[標題](網址) 點擊直接跳轉，不帶任何多餘文字
                md_lines.append(f"{global_idx}. [{title}]({url})")
                global_idx += 1
            md_lines.append("")  # 分類間隔空行
...
def run_pipeline():
    ...
    distilled_items = distiller.distill(target_candidates, profile, top_k=7)
    ...
    generate_newsletter_file(distilled_items)

    # 6. 發送 Bark 推播
    notifier = SimpleBarkNotifier()
    notifier.send_digest(distilled_items)
