...
1. 彙整論述 (Overview) 撰寫鐵律 (極度嚴格，違者重寫):
   ...
   - 必須提煉出 1~2 句「具體工程結論或核心痛點洞察」：直接點出文章內文提到的技術突破點是什麼、解決了什麼具體架構/效能痛點。
2. 台灣在地化用語校正：
   - 必須使用台灣慣用詞彙：軟體工程師（非程序員）、快取（非緩存）、資料庫（非數據庫）、伺服器（非服務器）、影片（非視頻）、資訊（非信息）、網路（非網絡）、軟體/硬體（非軟件/硬件）、支援（非支持）、相容（非兼容）、使用者（非用戶）。
3. 標點符號規範：
